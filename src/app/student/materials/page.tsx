...
export default function StudentMaterialsPage() {
  ...
  const newMaterials = mockMaterials.filter(material => material.isNew);
    ...
    <DashboardLayout role="student">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-heading font-bold">Learning Materials</h1>
            <p className="text-muted-foreground">
              Access study materials and resources for your classes.
            </p>
          </div>
        </div>
        
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          <Card className="col-span-full md:col-span-2 lg:col-span-3 bg-gradient-to-r from-primary/5 to-primary/10">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {newMaterials.slice(0, 3).map(material => {
                  const subject = mockSubjects.find(s => s.id === material.subjectId);
                  
                  return (
                    <div key={material.id} className="flex items-start space-x-4 rounded-md border p-4">
                      <div className="flex h-12 w-12 items-center justify-center rounded-md bg-primary/10">
                        {getMaterialTypeIcon(material.type)}
                      </div>
                      <div className="space-y-1">
                        <h4 className="font-medium leading-none">{material.title}</h4>
                        <p className="text-sm text-muted-foreground">
                          {subject?.name || 'Unknown Subject'} • {material.teacher}
                        </p>
